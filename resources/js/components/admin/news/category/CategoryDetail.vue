                    ...
                    <div class="col-md-8 offset-md-2 card show border border-0 p-4">
                        ...
                        <form @submit.prevent="updateCategory">
                            ...
                            <div class="d-flex justify-content-center">
                                <button
                                    type="submit"
                                    class="btn btn-all-add-edit mr-2 mt-2"
                                    :disabled="isDisabled"
                                >
                                    Cập Nhật Danh Mục
                                </button>
...
export default {
    ...
    methods: {
        ...
        getCategory(gameId) {
            this.axios
                .get(`/api/admin/category/${gameId}`)
                .then((response) => {
                    if (response.data.status === 200) {
                        if (response.data.data) {
                            this.data = response.data.data;
                            this.data.parent_id = this.data.parent_id === null ? '' : this.data.parent_id;
        ...
        updateCategory() {
            ...
            this.axios
                .put(`/api/admin/category/${this.data.id}/update`, this.requestData)
                .then((response) => {
        ...
        getCategoryOpts() {
            this.axios
                .get("/api/admin/category/category-opts", {params: {without_id: this.$route.params.id}})
                .then((response) => {
                    if (
